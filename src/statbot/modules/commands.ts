    ...
    if (message.msg.startsWith(":")) {
        switch (message.msg) {
          ...
          case ":optout":
            ...
            return true
          case ":status":
            chat.mongo.checkStatus(message.uid).then((user: { uid:string, optin: boolean; }) => {
              if (user && user.optin) {
                chat.pushChatMsg(
                  {
                    username: chat.chatConfig.username,
            ...
            case ":stats":
                chat.mongo.playingHours(message.uid).then((hours: any) => {
                    ...
                    chat.pushChatMsg(
                        {
                            username: chat.chatConfig.username,
                            msg: `${message.username}, you have played ${Math.floor(hours[0].total/60/60/1000)}:${Math.floor(hours[0].total/60/1000)%60} hours of music.`,
                        },
                        chat.chatConfig.user
                    );
                });
